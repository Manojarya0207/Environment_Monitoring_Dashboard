...
import { MetricScene } from "../../three/scenes/MetricScene";
...
export function MetricDetailOverlay({ metric, onClose }: MetricDetailOverlayProps) {
  ...
  return (
    <div className="flex-1 glass rounded-3xl overflow-hidden p-8 border-white/5 relative flex flex-col xl:flex-row gap-8 animate-in fade-in zoom-in duration-300">
      <button 
        onClick={onClose}
      ...
      <div className="w-full xl:w-1/3 flex flex-col">
        <h3 className="text-lg font-medium mb-4">Live Representation</h3>
        <div className="flex-1 relative">
          <MetricScene metric={metric} />
          <div className="absolute bottom-4 left-4 right-4 glass p-3 rounded-xl text-xs text-center border-white/5">
            Real-time sensory model rendering
          </div>
        </div>
      </div>
    </div>
