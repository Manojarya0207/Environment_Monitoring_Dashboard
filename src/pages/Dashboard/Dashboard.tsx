...
import { FeatureTour } from "../../components/Effects/FeatureTour";
import {
  Thermometer,
  Droplets,
  Sun,
...
export function Dashboard() {
  ...
  const [onboardingActive, setOnboardingActive] = useState(
    isFirstLogin && localStorage.getItem("onboarding_completed") !== "true"
  );
  ...
  const cardsData = [
    { id: "card-air-temp", title: "Air Temperature", value: "24.5", unit: "°C", icon: <Thermometer /> },
    { id: "card-rel-humidity", title: "Relative Humidity", value: "45", unit: "%", icon: <Droplets /> },
    { title: "Solar Radiation", value: "850", unit: "W/m²", icon: <Sun /> },
    { title: "Atmospheric Pressure", value: "101.2", unit: "kPa", icon: <Gauge /> },
    { title: "Air Quality (PM2.5)", value: "22", unit: "ppm", icon: <Activity /> },
    { title: "Wind Speed", value: "12.4", unit: "m/s", icon: <Wind /> },
    { title: "Wind Direction", value: "NW", unit: "", icon: <Compass /> },
    { title: "Rainfall", value: "12", unit: "mm", icon: <CloudRain /> },
    { title: "UV Index", value: "6", unit: "", icon: <SunDim /> },
    { title: "Battery Level", value: "98", unit: "%", icon: <Battery /> },
  ];

  return (
    <motion.div 
      initial={onboardingActive ? { opacity: 0 } : { opacity: 1 }}
      animate={{ opacity: 1 }}
      transition={{ duration: 0.6 }}
      className="p-8 pb-24 min-h-full flex flex-col gap-8 relative overflow-hidden"
    >
      {/* Background weather particles & ambient glows */}
      <WeatherParticles />

      {/* Hero Header Section */}
      <div className="z-10 relative overflow-hidden rounded-3xl border border-sky-100/50 bg-gradient-to-r from-sky-50/60 via-white to-emerald-50/30 p-8 min-h-[160px] flex items-center shadow-[0_4px_20px_-4px_rgba(14,165,233,0.05)] dark:border-slate-800 dark:from-slate-900/90 dark:via-slate-950/60 dark:to-emerald-950/10">
        {/* CSS Animations */}
        <style>{`
      ...
      <motion.div 
        ...
        initial={onboardingActive ? "hidden" : "show"}
        animate="show"
        ...
        {cardsData.map((card) => (
          <motion.div id={card.id} key={card.title} variants={cardVariants}>
            <KPICard 
              ...
              isActive={selectedMetric === card.title} 
              isLoading={isLoading} 
            />
          </motion.div>
        ))}
      ...
      {/* Mandatory Onboarding Tour */}
      <AnimatePresence>
        {onboardingActive && (
          <FeatureTour onComplete={() => setOnboardingActive(false)} />
        )}
      </AnimatePresence>
    </motion.div>
  );
